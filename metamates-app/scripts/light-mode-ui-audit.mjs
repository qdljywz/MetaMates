#!/usr/bin/env node
/**
 * 视觉与对比度走查 — 浅色模式，模拟用户操作并检测文字/背景可读性
 *
 * 用法:
 *   node scripts/light-mode-ui-audit.mjs
 *   METAMATES_WORKSPACE=E:\MyM2 node scripts/light-mode-ui-audit.mjs
 */
...
const REPORT_PATH = path.join(ROOT, 'light-mode-ui-audit-report.json')
const SHOTS_DIR = path.join(ROOT, 'test-results', 'light-mode-audit')
...
async function runAudit() {
  ...
  try {
    ...
    await bootstrapAuditTheme(win, 'light')
    await sleep(500)

    const theme = await win.evaluate(() => document.documentElement.getAttribute('data-theme'))
    record('主题', 'data-theme 为 light', theme === 'light', `theme=${theme}`)
    ...
    for (const [testId, label, shot] of [
      ...
      if (await btn.count()) {
        ...
        if (open) {
          ...
          if (testId === 'activity-graph') {
            recordContrast('对比度', '图谱·详情侧栏', await measureContrast(win, '.graph-detail-panel'), 4.5)
            recordContrast('对比度', '图谱·底部统计', await measureContrast(win, '.graph-footer'), 4.5)
          }
