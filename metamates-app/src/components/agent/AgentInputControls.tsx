...
const AgentInputControls = memo(({
  ...
  const pillKey = useMemo(
    () => deriveAgentSessionPillKey({
      connectionStatus,
      isStreaming,
      hasInProgressTools,
      warmupPhase,
    }),
    [connectionStatus, isStreaming, hasInProgressTools, warmupPhase],
  )
  ...
  const showModel = connectionStatus === 'connected' && models.length > 0
